...
const COLUMNS = [
  ...
  { key: 'physDesc', label: 'Характеристика' },
  { key: 'description', label: 'Описание' },
  { key: 'grntiAbbs', label: 'Коды ГРНТИ' },
  { key: 'bbks', label: 'ББК' },
  { key: 'udcs', label: 'УДК' },
...
const fmtAuthors = (authors: Author[] | null | undefined) =>
  authors?.length
    ? authors
        .map(
          a =>
        ...
        <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
          {books.map(b => (
            <div
              key={b.id}
              className={clsx(
                'border rounded p-3 cursor-pointer transition bg-gray-200 hover:bg-gray-300',
              )}
